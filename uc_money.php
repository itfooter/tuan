...
$credits_CFG = array(
    '1' => array('title'=>'积分', 'unit'=>'' ,'field'=>'score'),
);
...
{
        if(file_exists(APP_ROOT_PATH."uc_client/data/cache/creditsettings.php"))
        {
                require_once APP_ROOT_PATH."uc_client/data/cache/creditsettings.php";
                $creditsettings = $_CACHE['creditsettings'];
                if(count($creditsettings)>0)
                {
                        foreach($creditsettings as $k=>$v)
                        {
                                $creditsettings[$k]['srctitle'] = $credits_CFG[$v['creditsrc']]['title'];

                        }
                        $allow_exchange = true;
                        $GLOBALS['tmpl']->assign("allow_exchange",$allow_exchange);
                }
        }
}
...
elseif($_REQUEST['act']=='incharge')
{
	...
	//输出支付方式
	$payment_list = $GLOBALS['db']->getAllCached("select * from ".DB_PREFIX."payment where is_effect = 1 and class_name <> 'Account' and class_name <> 'Voucher' and class_name <> 'tenpayc2c' and online_pay = 1 order by sort desc");			
...
elseif($_REQUEST['act']=='incharge_done')
{
	...
	$order['bank_id'] = intval($_REQUEST['bank_id']);
...
elseif($_REQUEST['act']=='exchange'){
    ...
    $GLOBALS['tmpl']->assign("exchange_data",$creditsettings);
    $GLOBALS['tmpl']->assign("exchange_json_data",json_encode($creditsettings));
    $GLOBALS['tmpl']->display("uc.html");
}
elseif($_REQUEST['act']=='doexchange'){
    if($allow_exchange)
		{
			...
			{
				$cfg = $creditsettings[addslashes(trim($_REQUEST['key']))];
                                 if($cfg)
				{	
