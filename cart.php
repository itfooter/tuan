...
if($_REQUEST['act']=='addcart')
{
	...
	$is_lottery = $GLOBALS['db']->getOneCached("select is_lottery from ".DB_PREFIX."deal where id = ".$id);
        $buy_type = $GLOBALS['db']->getOneCached("select buy_type from ".DB_PREFIX."deal where id = ".$id);
        $max_bought = $GLOBALS['db']->getOneCached("select max_bought from ".DB_PREFIX."deal where id = ".$id);
	...
	if(!$user_info&&$is_lottery==1)
	{
	 ...
	 if($buy_type==3&&$user_info){
       
            $ms_dir='./public/miaosha';
            if(!is_dir($ms_dir)){
                mkdir($ms_dir,0700);
                chmod($ms_dir, 0700);   
            }
            ...
            $file_name=$id.'.php';
            $file_dir=$ms_dir.'/'.$file_name;
            $num=file_get_contents($file_dir);
...
elseif($_REQUEST['act']=='check')
{
		...
		//开始验证关于属性的库存
		$deal_attr_ids = $GLOBALS['db']->getAll("select deal_id,attr,name,attr_str from ".DB_PREFIX."deal_cart where session_id = '".session_id()."' and user_id = ".$user_info['id']);
		foreach($deal_attr_ids as $row)
		{
			...
			if($attr_setting_str!='')
			{
				...
				{
					if($ajax==1)
					showErr($checker['info']." ".$GLOBALS['lang']['DEAL_ERROR_'.$checker['data']]."|".$row['deal_id']."|".$checker['attr'],$ajax,url_pack("cart"));
					else	
					showErr($checker['info']." ".$GLOBALS['lang']['DEAL_ERROR_'.$checker['data']],$ajax,url_pack("cart"));
				}
			}
		...
		else
		{
			...
			//查询总金额
			$cart_total_price = $GLOBALS['db']->getOne("select sum(total_price) from ".DB_PREFIX."deal_cart where session_id = '".session_id()."' and user_id = ".intval($GLOBALS['user_info']['id']));
			$delivery_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_cart as dc left join ".DB_PREFIX."deal as d on dc.deal_id = d.id where dc.session_id = '".session_id()."' and dc.user_id = ".intval($GLOBALS['user_info']['id'])." and d.is_delivery = 1");
...
elseif($_REQUEST['act']=='done')
{
	...
	//验证购物车
		...
		//开始验证关于属性的库存
		$deal_attr_ids = $GLOBALS['db']->getAll("select deal_id,attr,name,attr_str from ".DB_PREFIX."deal_cart where session_id = '".session_id()."' and user_id = ".$user_id);
		foreach($deal_attr_ids as $row)
		{
	...
	$order['user_name'] = $_SESSION['user_info']['user_name'];
	
	
	$coupon_mobile = htmlspecialchars(addslashes(trim($_REQUEST['coupon_mobile'])));
	if($coupon_mobile!='')
	{
		$_SESSION['user_info']['mobile'] = $coupon_mobile;
...
else
{
	...
	if($user_info)
	$cart_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cart where user_id = ".intval($user_info['id']));
	else
	$cart_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_cart where session_id = '".session_id()."' and user_id = ".intval($user_info['id']));
	if(!$cart_list)
	{
	...
	//输出抽奖验证过的用户手机号
	$lottery_mobile = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".intval($user_info['id']));
