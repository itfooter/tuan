...
$act = trim(htmlspecialchars(addslashes($_REQUEST['act'])));
//显示的类型
if($act == 'history')
{
...
if(app_conf("SHOW_DEAL_CATE")==1)
{
	...
	{		
		...
		$v['current'] = 1;
		if($act=='history'||$act=='notice')
		$v['url'] = url_pack("deals#".$act,$v['id'],$v['uname']);
		else 
		$v['url'] = url_pack("deals",$v['id'],$v['uname']);
		$deal_cates[] = $v;
	}
...
if($_REQUEST['search'])
{
	$search_code = unserialize(base64_decode($_REQUEST['search']));
        $search_code['se_name']=htmlspecialchars($search_code['se_name']);
	if($search_code['se_name']!='')
	{
		$condition.=" and name like '%".$search_code['se_name']."%'";
		$supplier_ids=$GLOBALS['db']->getAll("select id from ".DB_PREFIX."supplier where name like '%".$search_code['se_name']."%'");
		if($supplier_ids)
		{
			...
			$condition.=" or supplier_id in (".implode(",",$sp_ids).") ";
		}
