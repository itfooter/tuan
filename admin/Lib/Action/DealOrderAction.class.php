...
class DealOrderAction extends CommonAction{
	...
	public function export_csv($page = 1)
	{
		...
		if($list)
		{
	    	...
	    	if($page == 1)
	    	{
		    	$content = iconv("utf-8","gbk","订单编号,用户名,套餐名称,订购数量,支付状态,发货状态,额外状态,售后,下单时间,订单总额,已收金额,收货人,发货地址,邮编,用户邮件,手机号码,订单留言");	    		    	
		    	$content = $content . "\n";
	    	}
	...
	public function foreverdelete() {
		...
		if (isset ( $id )) {
				...
				if ($list!==false) {
					...
					M("DealCoupon")->where(array ('order_id' => array ('in', explode ( ',', $id ) ) ))->delete(); //删除相关套餐券
	...
	public function view_order()
	{
		...
		//输出退款申请列表
		$order_refund_part = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."deal_order WHERE order_status <> 1 and id=".$id);
		foreach($order_refund_part as $k=>$v)
		{
				//若是部分支付，则显示付款总额
				if($order_refund_part[$k]['pay_status'] == 1)
				{
					$order_refund_item = $GLOBALS['db']->getOne("SELECT edoi.* FROM ".DB_PREFIX."deal_order_item edoi  where edoi.order_id=".$id." and edoi.buy_status = 1");
					if($order_refund_item)
						$this->assign("order_refund_item",$order_refund_part);
					else
						$this->assign("order_refund_item",null);
				}
				//显示未退款数据
				else if($order_refund_part[$k]['pay_status'] == 2)
				{
					$order_refund_item = $GLOBALS['db']->getAll("SELECT edoi.* FROM ".DB_PREFIX."deal_order_item edoi  where edoi.order_id=".$id." and edoi.buy_status = 1");
					foreach($order_refund_item as $kk=>$vv)
						$order_refund_item[$kk]['pay_status'] = 2;
					$this->assign("order_refund_item",$order_refund_item);
				}
		}
		//输出订单留言
