...
class PromoteMsgListAction extends CommonAction{
	...
	private function url_pack($url,$id = 0)
	{
		...
		$deal_city = $GLOBALS['db']->getRowCached("select id,name,uname from ".DB_PREFIX."deal_city where is_delete = 0 and is_effect = 1 and is_default = 1");
		$arr = explode("#",$url);
		$module = $arr[0];
		$action = $arr[1];
