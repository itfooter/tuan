<html>
			...
			KE.event.ready(function() {
				...
				KE.event.add(viewServer, 'click', function () {
					...
					fileManager = new KE.dialog({
						id : id,
						cmd : 'file_manager',
						file : 'file_manager/file_manager.php?id=' + id +'&root=<?php echo $_REQUEST['root'];?>',
						width : 500,
						height : 400,
						loadingMode : true,
						title : '浏览服务器',
	...
	<?php 
	error_reporting(0);
	$file_url = $_REQUEST['file']?trim($_REQUEST['file']):'';
	...
	<body>
		<div class="main">
			...
			<form name="uploadForm" method="post" enctype="multipart/form-data" target="uploadIframe" action="<?php echo $_REQUEST['root'];?>">
				<input type="hidden" id="editorId" name="id" value="<?php echo $_REQUEST['id'];?>" />
				<input type="hidden" id="referMethod" name="referMethod" value="" />
				<input type="hidden" name="imgBorder" value="0" />
				<input type="hidden" name="<?php echo $_REQUEST['var_module'];?>" value="File" />
				<input type="hidden" name="<?php echo $_REQUEST['var_action'];?>" value="do_upload_img" />
				<ul class="table">
					<li>
