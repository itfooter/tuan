...
if($_REQUEST['act']=='index')
{
	...
	$result = get_voucher_list($limit,$user_info['id']);
...
elseif($_REQUEST['act']=='do_incharge')
{
...
elseif($_REQUEST['act']=='do_exchange')
{
	...
	else
	{
		...
		else
		{
			...
			$rs = send_voucher($ecv_type['id'],$user_info['id'],1);
			if($rs)
			{
				require_once APP_ROOT_PATH."system/libs/user.php";
				$msg = sprintf($GLOBALS['lang']['EXCHANGE_VOUCHER_USE_SCORE'],$ecv_type['name'],$ecv_type['exchange_score']);
				modify_account(array('money'=>0,'score'=>"-".$ecv_type['exchange_score']),$user_info['id'],$msg);
