...
function check_deal_number_attr($id,$attr_setting,$number=0)
{
	...
	//2. 本团购当前会员的购物车中数量
	$deal_user_cart_count = intval($GLOBALS['db']->getOne("select sum(number) from ".DB_PREFIX."deal_cart where session_id='".session_id()."' and deal_id =".$id." and user_id = ".intval($GLOBALS['user_info']['id'])." and attr_str like '%".$stock_attr_setting."%'"));
	//3. 本团购当前会员未付款的数量
