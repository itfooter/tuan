...
class AppTemplate
{
    ...
    function display($filename, $cache_id = '')
    {
        ...
        foreach($layout_array[2] as $adv_id)
        {        	
        	$adv = $GLOBALS['db']->getRowCached("select * from ".DB_PREFIX."adv where tmpl='".app_conf("TEMPLATE")."' and adv_id='".$adv_id."' and is_effect = 1");      	
        	if($adv['city_ids']!='')
        	{
        		$city_ids = explode(',',$adv['city_ids']);
        		if(in_array($GLOBALS['deal_city']['id'],$city_ids))
        		{
        			$out = preg_replace("/<adv(\s+)adv_id=\"".$adv_id."\"([^>]*)>/i",$adv['code'],$out);
        			 //对图片路径的修复
			        $domain = app_conf("PUBLIC_DOMAIN_ROOT")==''?get_domain().$GLOBALS['IMG_APP_ROOT']:app_conf("PUBLIC_DOMAIN_ROOT");
			        $out = str_replace($GLOBALS['IMG_APP_ROOT']."./public/",$domain."/public/",$out);	
			        $out = str_replace("./public/",$domain."/public/",$out);	
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'include':
                    ...
                    {
                    	//强行编译后写入原编译文件，而不用include
                    	$complie_file = $this->compile_dir . '/' . basename($t[file]) . '.php';
                    	if(file_exists($complie_file))
                    	{
                    		return file_get_contents($complie_file);
                    	}
                    	else
                    	{
                    		$filename = $this->template_dir . '/' . $t[file];
                    		$this->make_compiled($filename);
                    		return file_get_contents($complie_file);
                    	}
