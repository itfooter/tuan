...
class QXT_sms implements sms
{
	...
	{		
		@session_start();
		$last_visit = intval($_SESSION['last_visit_qxt']);
		if(get_gmtime() - $last_visit > 10)
		{
			...
			$str = "短信平台&nbsp;&nbsp;剩余：".$result['body']."条";
		
			$_SESSION['qxt_info'] = $str;
			$_SESSION['last_visit_qxt'] = get_gmtime();
			return $str;
		}
		else
		{
			if($_SESSION['qxt_info'])
			return $_SESSION['qxt_info'];
			else
			return "通短信平台";
		}
