...
class Yeepay_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$data_return_url = get_domain().APP_ROOT.'/payment.php?act=return&class_name=Yeepay';
	...
	public function response($request)
	{
    	...
    	//@file_put_contents("./ealipay_".date("Y-m-d_H_i_s").".txt",$ScrtStr);
       // @file_put_contents("./ealipay_55.txt",$cs);
		if (strtoupper($mac) == strtoupper($mymac))
		{		
			if($succeed=="1")	
			{
                           // @file_put_contents("./ealipay_1.txt",1);
				...
				if($rs)
				{
                                    //@file_put_contents("./ealipay_2.txt","2");
                                    $rs = order_paid($payment_notice['order_id']);				
					if($rs)
					{
                                           // @file_put_contents("./ealipay_3.txt","3");
					...
					{
                                            //@file_put_contents("./ealipay_4.txt","4");
						if($bType=="2"){echo "success";	exit;}
						if($order_info['pay_status'] == 2)
						{
                                                    @file_put_contents("./ealipay_5.txt","5");
							if($order_info['type']==0)
							app_redirect(url_pack("payment#done",$payment_notice['order_id'])); //支付成功
							else
							app_redirect(url_pack("payment#incharge_done",$payment_notice['order_id'])); //支付成功
                                                   ...
                                                   // @file_put_contents("./ealipay__6.txt","6");
						app_redirect(url_pack("payment#pay",$payment_notice['id'])); 
					}
				}
				else
				{
                                    //@file_put_contents("./ealipay_ss.txt","success");
					if($bType=="2"){echo "success";	exit;}
					app_redirect(url_pack("payment#pay",$payment_notice['id'])); 
				}
			}
			...
			{
                           // @file_put_contents("./ealipay_998.txt",$GLOBALS['payment_lang']["PAY_FAILED"]);
				showErr($GLOBALS['payment_lang']["PAY_FAILED"]);
			}
		}else{
                   // @file_put_contents("./ealipay_999.txt",$GLOBALS['payment_lang']["PAY_FAILED"]);
		    showErr($GLOBALS['payment_lang']["PAY_FAILED"]);
		}   
	}
	...
	{
		$payment_item = $GLOBALS['db']->getRowCached("select * from ".DB_PREFIX."payment where class_name='Yeepay'");
		if($payment_item)
		{
