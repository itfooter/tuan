...
class Alipay_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$data_return_url = get_domain().APP_ROOT.'/payment.php?act=response&class_name=Alipay';
		...
		$payLinks = '<a onclick="window.open(\'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.$sign_md5.'&sign_type=MD5\')" href="javascript:;"><input type="submit" class="paybutton" name="buy" value="'.$GLOBALS['payment_lang']['GO_TO_PAY'].'"/></a>';
	...
	public function auto_do_send_goods($payment_notice,$order_info)
	{
		...
		{			
			...
			{
				$this->do_send_goods($payment_notice['id'],"团购订单".$order_info['order_sn']."支付成功，无需配送");
			}
		}
	}
	...
	public function do_send_goods($payment_notice_id,$delivery_notice_sn)
	{
		...
		$result = toArray($result,"alipay");
                //file_put_contents(APP_ROOT_PATH."/public/ealipay_".date("Y-m-d_H_i_s").".txt",$result);
		if($result['is_success'][0]=='T')
		{
                     //file_put_contents(APP_ROOT_PATH."/public/ealipay_1.txt","1");
			return "支付宝发货成功";
		}
		else
		{
			...
			{       
                            //file_put_contents(APP_ROOT_PATH."/public/ealipay_2.txt",ILLEGAL_ARGUMENT);
				return $result['error'].' 参数不正确';
			}
			elseif($result['error']=='TRADE_NOT_EXIST')
			{        
                            //file_put_contents(APP_ROOT_PATH."/public/ealipay_3.txt",TRADE_NOT_EXIST);
				return $result['error'].' 交易单号有误';
			}
			elseif($result['error']=='GENERIC_FAILURE')
			{      
                            //file_put_contents(APP_ROOT_PATH."/public/ealipay_4.txt",GENERIC_FAILURE);
				return $result['error'].' 执行命令错误';
			}
			else
			{      
                            //file_put_contents(APP_ROOT_PATH."/public/ealipay_5.txt",GENERIC_FAILURE);
				return $result['error'];
			}			
		}
	}
