...
class AlipayBank_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$data_return_url = get_domain().APP_ROOT.'/payment.php?act=return&class_name=AlipayBank';
		$data_notify_url = get_domain().APP_ROOT.'/payment.php?act=notify&class_name=AlipayBank';
		...
		$payLinks = '<a onclick="window.open(\'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.$sign_md5.'&sign_type=MD5\')" href="javascript:;"><input type="submit" class="paybutton" name="buy" value="'.sprintf($this->payment_lang['GO_TO_PAY'],$this->get_name($bank_type)).'"/></a>';
	...
	{		
		$payment_item = $GLOBALS['db']->getRowCached("select * from ".DB_PREFIX."payment where class_name='AlipayBank'");
		if($payment_item)
		{
