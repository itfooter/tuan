...
//查询用户收藏列表
function get_collection_list($limit,$user_id)
{
    $sql = "select * from ".DB_PREFIX."collection as c left join ".DB_PREFIX."deal as d on c.deal_id = d.id where c.user_id = ".$user_id." order by c.id desc limit ".$limit;

    $sql_count = "select count(id) from ".DB_PREFIX."collection where user_id = ".$user_id;
